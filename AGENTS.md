...
│   ├── lib/download-utils.sh     # Shared shell helpers: download_file, download_and_verify, verify_sha256/512, github_latest_*
│   ├── scripts/
...
- Every `<env>/docker-compose.yml` names its (single) service `dev`, not the env
  name. `run.sh` names the container after the project (`dev-<8 hex>`) through the
...
- **Versions default to "latest stable" at build time**
